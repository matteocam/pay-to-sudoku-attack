...
extern "C" {
    ...
    fn malicious_gen_keypair(n: uint32_t, h: *mut c_void, cb: extern fn(*mut c_void, *const c_char, size_t, *const c_char, size_t));
    ...
    fn malicious_snark_verify(keypair: *const Keypair,
                    n: uint32_t,
                    proof: *const uint8_t,
                    proof_len: int32_t,
    ...
    fn prove_malicious_verify(keypair: *const Keypair, h: *mut c_void,
                 cb: extern fn(*mut c_void, uint32_t, *const uint8_t, *const c_char, int32_t), 
                 n: uint32_t, puzzle: *const uint8_t, solution: *const uint8_t,
                 key: *const uint8_t, h_of_key: *const uint8_t) -> bool;
}
...
pub fn generate_keypair<F: for<'a> FnMut(&'a [u8], &'a [u8])>(num: usize, mut f: F) {
    ...
    unsafe {
        malicious_gen_keypair(num as u32, (&mut cb) as *mut _ as *mut c_void, handle_keypair_callback);
    }
}
...
pub fn evil_test<F: for<'a> FnMut(&'a [u8], &'a [u8])>(ctx: &Context, puzzle: &[u8], solution: &[u8], key: &[u8], h_of_key: &[u8], mut f: F) -> bool {
    let mut cb: &mut for<'a> FnMut(&'a [u8], &'a [u8]) = &mut f;
    ...
    unsafe {
        prove_malicious_verify(ctx.keypair, (&mut cb) as *mut _ as *mut c_void, handle_proof_callback, ctx.n as u32, &puzzle[0], &solution[0], &key[0], &h_of_key[0])
    }
}
...
pub fn verify(ctx: &Context, proof: &[u8], puzzle: &[u8], h_of_key: &[u8], encrypted_solution: &[u8]) -> bool
{
    ...
    unsafe { malicious_snark_verify(ctx.keypair, ctx.n as u32, &proof[0], proof.len() as int32_t, &puzzle[0], &h_of_key[0], &encrypted_solution[0]) }
}
